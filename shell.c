...
int
run_command (const char *command, char *output, int output_size)
{
	      ...
	      else
		{
		  ...
		  while (fgets (line, MAX_COMMAND_LENGTH, file) != NULL)
		      ...
		      if (strlen (line) > 0)
			{
			  ...
			  int line_result =
			    run_command (line, output, output_size);
			  if (line_result != 0)
			    {
  ...
  // reset stdin to its original file descriptor
  if (dup2 (STDIN_FILENO, fileno (stdin)) == -1)
    {
      perror ("dup2");
      result = 1;
    }
